...
async function joinRoomAsPlayer (roomId, user) {
...
async function joinRoomAsSpectator (roomId, user) {
    let room = await Room.findOne({_id: roomId});
    let ind = room.spectators.findIndex((element, index, array) => (element._id.toString() === user._id));
    if (ind === -1) {
...
module.exports = {getRooms, getPlayers, joinRoomAsPlayer, joinRoomAsSpectator, leaveRoom, createRoom, deleteRoom, getRoomById, addMessage}