...
class DurakGame{
    ...
    leaveGame(userId) {
        ...
        if (ind !== -1) {
            this.room.players.splice(ind, 1);
            this.discardHand(userId);
            return;
        }
        ind = this.room.spectators.findIndex((element, index, array) =>
            (element._id.toString() === userId.toString()));
        if (ind !== -1) {
            this.room.spectators.splice(ind, 1);
        }
    }

    joinAsSpectator(user) {
        this.room.spectators.push(user);
    }

    joinGameFromSpectator(userId) {
        let ind = this.room.spectators.findIndex((element, index, array) =>
            (element._id.toString() === userId.toString()));
        if (ind !== -1) {
            this.room.players.push(this.room.spectators[ind]);
            this.room.spectators.splice(ind, 1);
        }
    }
